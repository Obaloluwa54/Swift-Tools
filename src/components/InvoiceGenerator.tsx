...
import { Download, Plus, Trash2, Printer, ArrowLeft, MessageCircle, Layout, Globe, Phone, Mail } from 'lucide-react';
...
export default function InvoiceGenerator({ onBack }: { onBack: () => void }) {
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
        ...
        <div className="lg:col-span-5 space-y-6 no-print">
          ...
          <div className="bg-white p-6 rounded-2xl border border-zinc-100 shadow-sm space-y-4">
            <h3 className="text-lg font-semibold text-zinc-900 mb-4">Client & Info</h3>
        ...
        <div className="lg:col-span-7">
          <div className="sticky top-6">
            <div 
              ref={invoiceRef}
              className={cn(
                "bg-white p-6 sm:p-12 shadow-2xl border border-zinc-100 min-h-[800px] flex flex-col transition-all duration-500",
            ...
            >
                ...
                <>
                  <div className="flex justify-between items-start mb-16">
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <h1 className="text-4xl font-bold text-zinc-900 mb-2 uppercase tracking-tighter">Invoice</h1>
                        <p className="text-zinc-500 text-sm">{data.invoiceNumber}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <h2 className="font-bold text-zinc-900">{data.businessName}</h2>
                      <p className="text-zinc-500 text-sm whitespace-pre-line max-w-[250px] ml-auto">{data.businessAddress}</p>
                  ...
                  <div className="grid grid-cols-2 gap-12 mb-16">
                    <div>
                      <p className="text-xs uppercase tracking-widest text-zinc-400 font-semibold mb-2">Bill To</p>
                      <p className="font-bold text-zinc-900 text-lg">{data.customerName}</p>
                      <p className="text-zinc-500">{data.clientEmail}</p>
                    </div>
                    <div className="text-right grid grid-cols-2 gap-x-8 gap-y-2">
                      <p className="text-xs uppercase tracking-widest text-zinc-400 font-semibold">Date</p>
                      <p className="font-medium text-zinc-900">{data.date}</p>
                      <p className="text-xs uppercase tracking-widest text-zinc-400 font-semibold">Due Date</p>
                      <p className="font-medium text-zinc-900">{data.dueDate}</p>
                    </div>
                  </div>
                    ...
                    <tbody>
                      {data.items.map((item, index) => (
                        <tr key={index} className="border-b border-zinc-50">
                          <td className="py-6 text-zinc-700">{item.description || 'No description'}</td>
                          <td className="py-6 text-right text-zinc-900 font-medium">{data.currency}{item.amount.toFixed(2)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              {data.template === 'classic' && (
                <>
                  <div className="text-center mb-12 border-b-4 border-zinc-900 pb-8">
                    {data.logoUrl && (
                      <div className="w-24 h-24 mx-auto mb-4 grayscale opacity-80">
                        <img src={data.logoUrl} alt="Logo" className="w-full h-full object-contain" referrerPolicy="no-referrer" />
                      </div>
                    )}
                    <h2 className="text-3xl font-serif font-bold text-zinc-900 mb-2">{data.businessName}</h2>
                    <p className="text-zinc-600 italic">{data.businessAddress}</p>
                    <div className="mt-3 flex items-center justify-center gap-6 text-xs text-zinc-500 italic">
                  ...
                  <div className="flex justify-between mb-12">
                    <div>
                      <p className="text-xs uppercase tracking-widest text-zinc-400 font-semibold mb-2">Invoice To</p>
                      <p className="font-serif font-bold text-xl">{data.customerName}</p>
                      <p className="text-zinc-600">{data.clientEmail}</p>
                    </div>
                    <div className="text-right">
                  ...
                  <div className="border-y-2 border-zinc-900 py-6 mb-12">
                    <div className="grid grid-cols-12 gap-4 font-bold uppercase text-sm mb-6">
                      <div className="col-span-8">Description</div>
                      <div className="col-span-4 text-right">Amount</div>
                    </div>
                    {data.items.map((item, index) => (
                      <div key={index} className="grid grid-cols-12 gap-4 mb-4">
                        <div className="col-span-8 font-serif">{item.description}</div>
              ...
              <div className="mt-auto">
                <div className="flex justify-end mb-12">
                  <div className="w-64 space-y-3">
                    ...
                    <div className={cn(
                      "flex justify-between font-bold text-xl pt-3 border-t",
                      data.template === 'classic' ? "border-zinc-900" : "border-zinc-100"
                    )}>
                      <span>Total</span>
                ...
                <div className={cn(
                  "grid grid-cols-1 sm:grid-cols-2 gap-8 pt-8 border-t",
                  data.template === 'classic' ? "border-zinc-900" : "border-zinc-100"
                )}>
                  <div>
                    <p className="text-xs uppercase tracking-widest text-zinc-400 font-semibold mb-2">Payment Info</p>
                    <p className="text-sm text-zinc-700">{data.paymentMethod}</p>
                  </div>
                  {data.notes && (
                    <div className="sm:text-right">
                      <p className="text-xs uppercase tracking-widest text-zinc-400 font-semibold mb-2">Notes</p>
